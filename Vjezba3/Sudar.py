...
# Bisection
def bisekcija(a, b, tol=1e-10, Nmax=200):
    fa = f(a)
    fb = f(b)
    if fa * fb > 0:
    ...
    for i in range(1, Nmax+1):
        c = 0.5 * (a + b)
        fc = f(c)
        print("{:2d} {:.6f}".format(i, c))  # print iteration number and c as in the PDF
...
# Newton-Raphson
def newton(x0, tol=1e-12, Nmax=100):
    x = x0
    for i in range(1, Nmax + 1):
        fx = f(x)
        dfx = df(x)
...
print("Bisekcija: t = {:.12f}, iteracije = {}, y = {:.12f}".format(t_bis, iter_bis, y_at_t_bis))
print("Newton:    t = {:.12f}, iteracije = {}, y = {:.12f}".format(t_newt, iter_newt, y_at_t_newt))
